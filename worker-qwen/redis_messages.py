...
def _optional_bool(value: Any) -> bool:
    """JSON 은 bool 을 그대로 싣지만, 문자열로 오는 클라이언트도 받아 준다."""
...
class TtsPayload:
    ...
    speaker_blob_key: str | None = None
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "TtsPayload":
        return cls(
            ...
            # 게이트웨이는 speaker* 만 보낸다. style* 는 이 이름으로 발행된 in-flight 잡을 위한
            # 전환용 fallback 이라, 큐가 비고 나면 지워도 된다.
            speaker_name=_optional_text(
                data.get("speakerName") or data.get("speaker_name")
                or data.get("speakerId") or data.get("speaker_id")
                or data.get("styleId") or data.get("style_id")
            ),
            reference_digest=_optional_text(data.get("referenceDigest") or data.get("reference_digest")),
            speaker_blob_key=_optional_text(
                data.get("speakerBlobKey") or data.get("speaker_blob_key")
                or data.get("styleBlobKey") or data.get("style_blob_key")
            ),
            speaker_mode=_optional_text(
                data.get("speakerMode") or data.get("speaker_mode")
                or data.get("styleMode") or data.get("style_mode")
            ),
            speaker_ref_text=_optional_text(
                data.get("speakerRefText") or data.get("speaker_ref_text")
                or data.get("styleRefText") or data.get("style_ref_text")
            ),
            ...
            do_sample=None if data.get("do_sample") is None else _optional_bool(data.get("do_sample")),
            temperature=_optional_float(data.get("temperature")),
            top_p=_optional_float(data.get("top_p")),
            top_k=_optional_int(data.get("top_k")),
            repetition_penalty=_optional_float(data.get("repetition_penalty")),
            max_new_tokens=_optional_int(data.get("max_new_tokens")),
            subtalker_dosample=None if data.get("subtalker_dosample") is None else _optional_bool(data.get("subtalker_dosample")),
            subtalker_temperature=_optional_float(data.get("subtalker_temperature")),
            subtalker_top_p=_optional_float(data.get("subtalker_top_p")),
            subtalker_top_k=_optional_int(data.get("subtalker_top_k")),
        )
...
@dataclass(frozen=True, slots=True)
class SpeakerForgetPayload:
    speaker_name: str

    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> "SpeakerForgetPayload":
        value = data.get("speakerName") or data.get("speaker_name") or data.get("speakerId")
        if value is None:
            raise RedisModelError("missing 'speakerName' in payload")
        return cls(speaker_name=str(value))


JobPayload: TypeAlias = TtsPayload | EmptyPayload | SpeakerForgetPayload
...
def parse_job_payload(job_type: JobType, data: Mapping[str, Any]) -> JobPayload:
    model = {
        ...
        JobType.SPEAKER_FORGET: SpeakerForgetPayload,
    }[job_type]
...
ResultPayload: TypeAlias = AudioResult | StylesResult | SpeakerControlResult
